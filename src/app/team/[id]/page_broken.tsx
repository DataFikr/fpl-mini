...
function renderTeam2611652() {
  ...
  return renderTeamPage(team, managerData, leagues, currentGameweek, currentGWPoints, totalPoints);
}

// Generic team renderer for all other teams
function renderGenericTeam(teamId: number) {
  ...
  const managerData = {
    player_first_name: team.managerName.split(' ')[0],
    player_last_name: team.managerName.split(' ')[1],
    favourite_team: null,
    player_region_name: "England",
    player_region_iso_code_short: "EN",
    summary_overall_points: Math.floor(Math.random() * 300) + 200,
    summary_overall_rank: Math.floor(Math.random() * 5000000) + 100000
  };

  const currentGameweek = 5;
  const currentGWPoints = Math.floor(Math.random() * 50) + 30;
  const totalPoints = managerData.summary_overall_points;

  // Generate generic leagues for this team
  const leagues = [
    {
      id: 999000 + teamId,
      name: "Overall League",
      ...
      standings: [{
        ...
        teamName: team.name,
        managerName: team.managerName,
        rank: Math.floor(Math.random() * 1000000) + 100000,
        points: totalPoints,
        gameweekPoints: currentGWPoints
      }]
    },
...
// Common team page renderer
function renderTeamPage(team: any, managerData: any, leagues: any[], currentGameweek: number, currentGWPoints: number, totalPoints: number) {
  return (
                ...
                <div className="flex flex-wrap gap-6 text-base">
                  ...
                  <div className="flex items-center bg-gradient-to-r from-gray-50 to-slate-50 px-4 py-2 rounded-xl border border-gray-200">
                    ...
                    <div>
                      ...
                      <div className="text-gray-700 font-mono">{team.id}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-green-500 to-blue-500 p-1 rounded-3xl">
                <div className="w-32 h-32 bg-white border-4 border-white rounded-2xl flex items-center justify-center">
                  <div className="bg-gradient-to-r from-green-600 to-blue-600 w-24 h-24 rounded-xl flex items-center justify-center">
                    <span className="text-white text-lg font-bold">{team.name.slice(0, 2).toUpperCase()}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-5 gap-6 mb-8">
            ...
            <StatCard
              ...
              value={`#${Math.min(...leagues.map(l =>
                l.standings.find((s: any) => s.teamId === team.id)?.rank || 999
              ))}`}
...
export default async function TeamPage({ params }: TeamPageProps) {
  try {
    ...
    return await renderTeamPageWrapper(teamId);
  } catch (error) {
...
async function renderTeamPageWrapper(teamId: number) {
